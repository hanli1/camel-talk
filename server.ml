  ...
  if meth = "POST" then
    try
      ...
      if res = true then {status_code=200; response_body="{\"status\":\"success\",\"message\"" ^
        ":\"Voted on poll successfully\"}"}
      else {status_code=200; response_body="{\"status\":\"failure\",\"message\"" ^
        ":\"Cannot vote on poll\"}"}
    with
      | _ ->
        {status_code=400; response_body="{\"status\":\"failure\",\"message\"" ^
  ...
  body |> Cohttp_lwt_body.to_string >>= (
    fun body ->
      ...
      let response_result =
        ...
        else if uri_path = "/vote_poll" then
          vote_poll_api {request_info=req; request_body=body}
        else
